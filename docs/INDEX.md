...
Cloudflare反映担当は最初に`final-stage08/00_INDEX.md`を読みます。現行versionは`0.8.10`です。migrationは`0017`までです。既存のStage 8.2反映手順は`final-stage08/20_CODEX_DEPLOY_INSTRUCTION_FINAL.md`に保存しています。v0.8.9の変更点は`v0.8.9-usability-release.md`。v0.8.10の修正点は`v0.8.10-debug-fixes.md`を参照します。以下の段階別資料は履歴です。
...
| `versioning-policy.md` | CPCV独自のバージョン命名規則 |
