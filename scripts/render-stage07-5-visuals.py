...
OUT = ROOT / "docs" / "stage07-5-screenshots"
...
async def prepare(page) -> None:
    ...
    () => {
      ...
      document.querySelector('#organizationFilterStatus').textContent = '4語を登録中。上限2000語。';
      const categories = [
      ...
      const terms = [
        ['ちんこ','下ネタ・性的表現','3','使用',true],['ばかやろう','侮辱・嫌がらせ','4','使用',true],
        ['露骨危険語','下ネタ・性的表現','5','使用',true],['政府批判','政治的発言','5','使用',false]
      ];
      const termsBody = document.querySelector('#filterTermsBody');
      for (const item of terms) {
        const row=termsBody.insertRow();
        for (const value of item.slice(0,4)) { const cell=row.insertCell(); cell.textContent=value; }
        const enabled=row.insertCell(); enabled.innerHTML=`<input type="checkbox" ${item[4]?'checked':''} aria-label="有効">`;
        const action=row.insertCell(); action.innerHTML='<button class="button small danger">削除</button>';
      }
      const policies = {
      ...
      document.querySelector('#sessionFilterStatus').textContent = '有効。AI経路: 曖昧な場合のみ';
      ...
      const data=[
        ...
        ['2026/7/16 10:11','学生B','ばかやるう','','辞書: 承認待ち / 侮辱・嫌がらせ','承認待ち']
      ];
...
async def main() -> None:
    async with async_playwright() as p:
        ...
        await browser.close()
    print(json.dumps({'ok':True,'screenshots':5,'outputDir':str(OUT)},ensure_ascii=False))
