...
OUT = ROOT / "docs" / "stage07-6-screenshots"
...
async def prepare(page) -> None:
    ...
    () => {
      ...
      for (const [id,label] of [['und','自動・指定なし'],['ja','日本語'],['en','英語'],['th','タイ語']]) language.append(new Option(label,id));
      ...
      const terms = [
        ...
        ['คำหยาบ','タイ語','暴言・下品な表現','3','部分一致','不使用',true],['政府批判','日本語','政治的発言','5','自動','使用',false]
      ];
