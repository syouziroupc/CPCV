...
$stagingRuntimeConfigPath = Join-Path $projectRoot '.cpcv-staging.wrangler.toml'

Write-Host ''
Write-Host 'Class PDF Comment Viewer - production deployment' -ForegroundColor Yellow
Write-Host 'This command validates one clean staged commit, records read-only rollback information before approval, then migrates and deploys only after explicit confirmation.'
Write-Host ''
...
Copy-Item -LiteralPath $stagingConfigPath -Destination (Join-Path $recordRoot '00-wrangler-staging-canonical.toml')
...
try {
  Invoke-RecordedNative 'Materialize source-root staging config' '00-materialize-staging-config.txt' { & node scripts/materialize-staging-config.mjs $stagingConfigPath --expected-sha256 $stagingConfigHash }
  if (-not (Test-Path -LiteralPath $stagingRuntimeConfigPath -PathType Leaf)) { throw 'Runtime staging config was not created.' }
  if ((Get-FileHash -LiteralPath $stagingRuntimeConfigPath -Algorithm SHA256).Hash -ine $stagingConfigHash) { throw 'Runtime staging config does not match the canonical external config.' }

  Invoke-RecordedNative 'Source manifest verification' '00-verify-source-manifest.txt' { & npm run verify:source-manifest }
  ...
  Invoke-RecordedNative 'Staging configuration verification' '02-verify-staging-config.txt' { & node scripts/verify-deployment-config.mjs $stagingRuntimeConfigPath }
  Invoke-RecordedNative 'Production and staging separation verification' '03-verify-environment-separation.txt' { & node scripts/verify-environment-separation.mjs wrangler.toml $stagingRuntimeConfigPath }
  ...
  Invoke-RecordedNative 'Staging AI readiness' '11-staging-ai-readiness.txt' { & node scripts/verify-ai-readiness.mjs --config $stagingRuntimeConfigPath }
  Invoke-RecordedNative 'Production Wrangler dry-run' '12-production-dry-run.txt' { & npm run deploy:dry-run }
  Invoke-RecordedNative 'Staging Wrangler dry-run' '13-staging-dry-run.txt' { & npx wrangler deploy --dry-run --config $stagingRuntimeConfigPath }
  ...
  if ([string]::IsNullOrWhiteSpace($env:AUTH_RATE_LIMIT_PEPPER)) { throw 'Set AUTH_RATE_LIMIT_PEPPER in the current process before deployment.' }
  if ([string]::IsNullOrWhiteSpace($env:PUBLIC_RATE_LIMIT_PEPPER)) { throw 'Set PUBLIC_RATE_LIMIT_PEPPER in the current process before deployment.' }
  if ([string]::IsNullOrWhiteSpace($env:TURNSTILE_SECRET_KEY)) { throw 'Set TURNSTILE_SECRET_KEY in the current process before deployment.' }

  # Read-only Cloudflare preflight. No migration, secret write, deploy, rollback, or data mutation occurs before approval.
  ...
  Invoke-RecordedNative 'Remote DB_V2 verification before approval' '25-remote-d1-before-approval.txt' { & node scripts/verify-remote-d1.mjs }
  Invoke-RecordedNative 'Email authentication cutover readiness' '26-email-auth-readiness.txt' { & npm run verify:email-auth-ready }
  Invoke-RecordedNative 'Worker deployment status before approval' '27-deployments-status-before.txt' { & npx wrangler deployments status }
  Invoke-RecordedNative 'Worker version list before approval' '28-versions-list-before.txt' { & npx wrangler versions list }

  Write-Host ''
  Write-Host 'Review files 21 through 28 in the deployment record directory before approval.' -ForegroundColor Cyan
  Write-Host "Exact release commit: $releaseCommit" -ForegroundColor Cyan
  ...
  if ($confirmation -cne 'DEPLOY_PRODUCTION') {
    ...
    Write-Host 'Deployment cancelled. No Cloudflare mutation was performed.' -ForegroundColor Cyan
  } else {
    Invoke-RecordedNative 'Legacy D1 migration' '30-legacy-migrations-apply.txt' { & npx wrangler d1 migrations apply class_comment_db --remote }
    Invoke-RecordedNative 'DB_V2 migration' '31-v2-migrations-apply.txt' { & npx wrangler d1 migrations apply class_comment_db_v2 --remote }
    Invoke-RecordedNative 'Remote DB_V2 verification after migration' '32-remote-d1-after-migration.txt' { & node scripts/verify-remote-d1.mjs }
    Invoke-RecordedNative 'Authentication secret configuration' '33-secret-configuration.txt' { & node scripts/configure-rate-limit-secret.mjs }
    Invoke-RecordedNative 'Worker deployment' '34-worker-deploy.txt' { & npx wrangler deploy }
    Invoke-RecordedNative 'Remote DB_V2 verification after deploy' '35-remote-d1-after-deploy.txt' { & node scripts/verify-remote-d1.mjs }
    Invoke-RecordedNative 'Production smoke checks' '36-production-smoke.txt' { & node scripts/smoke-production.mjs }
    Invoke-RecordedNative 'Worker deployment status after deploy' '37-deployments-status-after.txt' { & npx wrangler deployments status }
    Invoke-RecordedNative 'Worker version list after deploy' '38-versions-list-after.txt' { & npx wrangler versions list }
    $deploymentResult = 'PASSED'
  }
} finally {
  Remove-Item -LiteralPath $stagingRuntimeConfigPath -Force -ErrorAction SilentlyContinue
  @(
  ...
  Get-ChildItem -LiteralPath $recordRoot -Recurse -File |
    ...
    ForEach-Object {
      $relativePath = [System.IO.Path]::GetRelativePath($recordRoot, $_.FullName).Replace('\', '/')
