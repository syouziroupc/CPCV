...
const ACCOUNT_ID = mustEnv('CLOUDFLARE_ACCOUNT_ID');
const API_TOKEN = mustEnv('CLOUDFLARE_API_TOKEN');
const DB_ID = String(process.env.STAGING_DB_V2_ID || '7b30a11d-5b3c-49f5-bc54-9a1326818089');
...
try {
  ...
  const hardFailures = invariantFailures(REPORT.invariants);
  if (hardFailures.length) throw new Error(`Invariant failures: ${hardFailures.join(', ')}`);
} catch (error) {
...
async function seedIsolatedSession() {
  ...
  await d1(`UPDATE organization_ai_settings
            SET enabled=1, moderation_daily_limit=5000, translation_daily_limit=5000,
  ...
  await d1(`UPDATE session_ai_settings SET moderation_enabled=1, translation_enabled=1,
            target_language='ja', translation_quality='fast', updated_by_user_id=?3, updated_at=?4
...
async function runAiPhase({ name, quality, count, concurrency, timeoutMs }) {
  ...
  const jobsBefore = await phaseJobCount(marker);
  if (jobsBefore !== 0) throw new Error(`${name}: marker collision`);
  const started = Date.now();
  const posts = await runPool(count, concurrency, async (i) => postComment(marker, i));
  const postElapsedMs = Date.now() - started;
  const accepted = posts.filter((item) => item.ok).length;
  const httpFailures = posts.filter((item) => !item.ok);
  const postLatencies = posts.map((item) => item.latencyMs);

  const phase = {
    name, quality, count, concurrency, accepted,
    postElapsedMs,
    postLatencyMs: distribution(postLatencies),
    httpStatusCounts: countBy(posts, (item) => String(item.status)),
    httpFailureSamples: httpFailures.slice(0, 10),
    ai: null
  };
  REPORT.phases.push(phase);
  if (accepted !== count) {
    throw new Error(`${name}: only ${accepted}/${count} comments were accepted`);
  }

  phase.ai = await waitForAi(marker, accepted * 2, timeoutMs);
  if (phase.ai.totalJobs !== accepted * 2) {
    throw new Error(`${name}: expected ${accepted * 2} AI jobs, found ${phase.ai.totalJobs}`);
  }
  if (phase.ai.nonTerminal !== 0) {
    throw new Error(`${name}: ${phase.ai.nonTerminal} AI jobs remained non-terminal after timeout`);
  }
  if (phase.ai.failed > 0) {
    throw new Error(`${name}: ${phase.ai.failed} AI jobs failed`);
  }
}
...
async function postComment(marker, i) {
  ...
  try {
    const response = await fetch(`${ORIGIN}/api/public/sessions/${PUBLIC_CODE}/messages`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'user-agent': 'CPCV-staging-load-test/1.0' },
      body: JSON.stringify({ nickname: `load-${i + 1}`, message, idempotencyKey: randomUUID() })
    });
    const body = await response.json().catch(() => null);
    return {
      ok: response.status === 201 && body?.ok === true,
...
async function waitForAi(marker, expectedJobs, timeoutMs) {
  ...
  snapshot = await aiSnapshot(marker);
  const rows = await d1(`SELECT j.job_type,j.status,j.attempt_count,j.last_error_code,j.model,
  ...
  const usage = await d1(`SELECT job_type,model,COUNT(*) AS calls,
      SUM(input_characters) AS input_characters,SUM(output_characters) AS output_characters
    FROM ai_usage_events WHERE job_id IN (
...
async function aiSnapshot(marker) {
  ...
  return {
    ...
    nonTerminal: Number(r.non_terminal || 0),
...
async function phaseJobCount(marker) {
  const rows = await d1(`SELECT COUNT(*) AS n FROM ai_jobs j JOIN comments c ON c.id=j.comment_id
    WHERE j.live_session_id=?1 AND c.message LIKE ?2`, [SESSION_ID, `%${marker}%`]);
...
async function runReadStorm({ count, concurrency }) {
  ...
  REPORT.phases.push({
    name: 'read_storm', count, concurrency,
    accepted: rows.filter((r) => r.ok).length,
    elapsedMs: Date.now() - started,
    latencyMs: distribution(rows.map((r) => r.latencyMs)),
    httpStatusCounts: countBy(rows, (r) => String(r.status)),
    failureSamples: rows.filter((r) => !r.ok).slice(0, 10)
  });
  if (rows.some((r) => !r.ok)) throw new Error(`read_storm: ${rows.filter((r) => !r.ok).length} requests failed`);
}
...
async function auditInvariants() {
  ...
  const stuck = Number((await d1(`SELECT COUNT(*) AS n FROM ai_jobs WHERE live_session_id=?1
    AND status IN ('queued','retry','processing')`, [SESSION_ID]))[0]?.n || 0);
  ...
  const seqCount = Number(sequence.n || 0);
  const seqDistinct = Number(sequence.distinct_n || 0);
  const seqMin = sequence.min_seq == null ? null : Number(sequence.min_seq);
  const seqMax = sequence.max_seq == null ? null : Number(sequence.max_seq);
  const gapless = seqCount === 0 || (seqDistinct === seqCount && seqMin === 1 && seqMax === seqCount);
  const quick = (await d1(`PRAGMA quick_check`))[0];
  const foreign = await d1(`PRAGMA foreign_key_check`);
  return {
    comments, expectedComments, jobs, expectedJobs, orphanJobs, badTranslations, stuck,
    realtimeEvents: seqCount, realtimeDistinctSequences: seqDistinct, realtimeMinSequence: seqMin,
    realtimeMaxSequence: seqMax, realtimeGapless: gapless,
    quickCheck: quick?.quick_check || quick?.integrity_check || Object.values(quick || {})[0] || null,
...
async function cleanup() {
  ...
  await d1(`UPDATE organizations SET status='deleted',updated_at=?2,deleted_at=?2
            WHERE id=?1 AND status='active'`, [ORG_ID, now]);
}
...
async function d1(sql, params = []) {
  ...
  const body = await response.json().catch(() => null);
  if (!response.ok || body?.success !== true) throw new Error(`D1 API ${response.status}: ${JSON.stringify(body)}`);
...
function distribution(values) {
  ...
  return {
    count: nums.length, min: nums[0], p50: at(0.50), p95: at(0.95), p99: at(0.99), max: nums.at(-1),
...
function renderMarkdown(report) {
  ...
  for (const phase of report.phases) {
    lines.push(`## ${phase.name}`, ``, '```json', JSON.stringify(phase, null, 2), '```', '');
  }
