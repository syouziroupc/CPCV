...
async function seed() {
  ...
  const sessionResult = await api('/api/private/sessions', { method: 'POST', body: { title: 'Stability audit class', moderationMode: 'off' } });
  ...
  for (let index = 0; index < messages.length; index += 1) {
    ...
    posted.push(body.commentId || body.id || '');
  }
  ...
  while (Date.now() < deadline) {
    ...
    jobRows = d1(`SELECT j.job_type, j.status, j.attempt_count, j.last_error_code, u.model
      FROM ai_jobs j LEFT JOIN ai_usage_events u ON u.job_id=j.id
    ...
    const terminal = jobRows.length >= 4 && jobRows.every((row) => ['succeeded','failed','skipped'].includes(String(row.status)));
...
async function cleanup() {
  let secret;
  try { secret = JSON.parse(readFileSync(secretPath, 'utf8')); } catch { return; }
