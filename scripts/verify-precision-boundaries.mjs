...
check("production deployment requires same-commit staging evidence", deploy.includes("staging_commit_sha") && deploy.includes("STAGING_PASSED") && deploy.includes("staging_test_record_sha256"));
...
check("production deployment records D1 bookmark and runs Stage 8.2 preflight", deploy.includes("d1 time-travel info class_comment_db_v2") && deploy.includes("npm run verify:stage82-preflight"));
check("production deployment records current Worker state", deploy.includes("wrangler deployments status") && deploy.includes("wrangler versions list"));
...
check("safe deploy requires clean same-commit staging evidence", safeDeploy.includes("git status --porcelain") && safeDeploy.includes("CPCV_STAGING_COMMIT_SHA") && safeDeploy.includes("CPCV_STAGING_TEST_RECORD_SHA256"));
check("safe deploy runs Stage 8.2 preflight and final docs validation", safeDeploy.includes("npm run verify:stage82-preflight") && safeDeploy.includes("npm run verify:final-docs"));
...
check("Stage 8.2 preflight and documentation verifiers exist", packageJson.scripts?.["verify:stage82-preflight"] === "node scripts/verify-stage82-preflight.mjs" && packageJson.scripts?.["verify:final-docs"] === "node scripts/verify-final-documentation.mjs");
check("final hardening regression is integrated", packageJson.scripts?.["test:final-hardening"] === "node --no-warnings scripts/test-final-hardening.mjs" && text("scripts/test-stage08-all.mjs").includes("scripts/test-final-hardening.mjs"));
...
for (const path of [
  ...
  "scripts/verify-remote-d1.mjs", "scripts/verify-stage82-preflight.mjs", "scripts/verify-final-documentation.mjs",
  "scripts/verify-email-auth-readiness.mjs", "scripts/configure-rate-limit-secret.mjs",
